...
mod imp {
	...
	pub fn get_product_name(path: &Path) -> Option<String> {
		...
		let mut unused = 0u32;
		let size = unsafe {
			GetFileVersionInfoSizeW(PCWSTR::from_raw(wide.as_ptr()), Some(&mut unused))
		};
		if size == 0 {
			return None;
		}
		...
		let lang =
			unsafe { std::slice::from_raw_parts(lang_ptr as *const u16, lang_len_u16) };
		let block = format!(
		...
		let slice = unsafe {
			std::slice::from_raw_parts(val_ptr as *const u16, val_len as usize - 1)
		};
