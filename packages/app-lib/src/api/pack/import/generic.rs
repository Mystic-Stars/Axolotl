...
pub async fn import_generic(
	...
	let dotminecraft = if dotminecraft.is_dir() {
		tracing::debug!("import_generic: using .minecraft subdir at {}", dotminecraft.display());
		dotminecraft
	} else {
		tracing::debug!("import_generic: using folder directly at {}", instance_folder.display());
		instance_folder
	};
	...
	if let Some(ref loader) = info.loader {
		...
		if let Some(dep) = dep {
			dependencies.insert(
				dep,
				info.loader_version.clone().unwrap_or_default(),
			);
		}
	}
	...
	finish_import(instance_id, dotminecraft, &state.io_semaphore, reporter, details).await
}
