...
fn find_json(path: &Path) -> Option<(String, String)> {
	...
	if json_files.len() == 1 {
		...
		let name = json_files[0]
			...
			.unwrap_or(name);
		return std::fs::read_to_string(&json_files[0]).ok().map(|c| (name, c));
	}
	if json_files.len() > 1 {
		debug!(
...
fn normalize_version(raw: &str) -> String {
	...
	if (v.starts_with("20.") || v.starts_with("21."))
		&& !v.starts_with("1.")
	{
		v = format!("1.{v}");
	}
...
fn extract_version_from_id(id: &str) -> Option<String> {
	...
	if ver.len() >= 32 && !ver.contains('.') && !ver.contains('-') && !ver.contains('_') {
		return None;
	}
...
fn detect_loader(content: &str, json: &Value) -> Option<(String, Option<String>)> {
