...
- Revert metadata is not hand-maintained. `scripts/axolotl/migration-revert.mjs` derives the columns a downgrade drops by parsing `ADD COLUMN` in each migration's SQL, for every migration at or after `OLDEST_REVERTIBLE_VERSION`. Do not reintroduce a mapping table in `scripts/axolotl/downgrade-app-db.mjs`: a hand table drifts the moment a migration is renumbered, which silently blocks the documented recovery.
- A migration that creates a table is not reversible by record removal alone. SQLite has no safe blanket `DROP TABLE` here, so a downgrade that would pass such a migration refuses at `--apply` until each created table is named in `AXOLOTL_DROP_CREATED_TABLES`. A migration that rebuilds an existing table (create scratch, drop old, rename onto the old name) is refused outright, because the surviving table holds rows that predate the migration and no mapping can restore them.
- If a migration adds durable schema that the derivation cannot describe, extend `scripts/axolotl/migration-revert.mjs` and its tests in the same change. Do not use `--allow-unmapped` as an implementation or CI workaround.
- After adding or changing a launcher migration or its revert derivation, run `node scripts/axolotl/downgrade-app-db.test.mjs` and `node scripts/axolotl/migration.test.mjs`; both are part of the CI guardrails. Add or update downgrade test coverage when introducing a new derivation shape or rollback behavior.
