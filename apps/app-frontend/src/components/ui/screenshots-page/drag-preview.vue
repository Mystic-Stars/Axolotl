...
<template>
	<div aria-hidden="true" class="relative w-full select-none">
		...
		>
			...
			<TagItem
				v-if="count > 1"
				class="!absolute right-3 top-3 z-[2] border-surface-5 bg-surface-4 tabular-nums text-[var(--color-text-tertiary)] motion-safe:transition-opacity motion-safe:duration-150 motion-safe:ease-out"
			>
				{{ count }}
			</TagItem>
		</div>
